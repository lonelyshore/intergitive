...
exports = module.exports = class GitStorage extends RepoStorage {
    ...
    load(revisionName, destinationPath, isTemplate) {
        ...
        .catch((err) => {
            if (!isBreaking) {
                console.error("Error occured when restoring " + sourcePath);
                console.error(err);
                isBreaking = true;
            }
    ...
    [_copyToTarget](sourcePath, isTemplate) {

        let copyWorkTree = () => {

            return fs.emptyDir(sourcePath)
            .then(() => {
        ...
        .catch((err) => {
            if (!isBreaking) {
                console.error("Error happened when trying to restore repo to " + sourcePath + " from " + this.backupRepoPath);
                isBreaking = true;
            }
        ...
        .catch((err) => {
            if (isBreaking) {
                console.error("Error happened when trying to restore index for " + sourcePath + " from " + this.indexFilePath);
                isBreaking = true;
            }
