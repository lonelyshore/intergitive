...
class Props {
    ...
    getRevisionRemoteSettingName(revisionName) {
...
exports = module.exports = class ArchiveRepoStorage extends RepoStorage {
    ...
    saveRepo(revisionName, sourcePath, isSaveTemplate) {
        ...
        function trySaveRemotePathSeparatedly(sourceConfigPath, temporaryPath, repoArchivePath, removeRemotePathsFromConfig) {
            return fs.access(sourceConfigPath)
            .then(() => {
                return hideRemotePath(sourceConfigPath, temporaryPath, repoArchivePath);
            }, () => {});
        }
    }
    ...
    loadLocal(revisionName, destinationPath, isTemplate) {
        return this.loadRepo(revisionName, destinationPath);
    }
    ...
    loadRepo(revisionName, destinationPath) {
        let archivePath = this.props.getRevisionArchivePath(revisionName);
        return fs.exists(archivePath)
        .then(archiveExists => {
    ...
    tryLoadRevisionIntoCache(revisionName) {
        ...
        let loadRevisionIntoCache = () => {
            ...
            .then(() => {
                return this.loadLocal(revisionName, cachingPath, false);
            })
            .then(() => {
