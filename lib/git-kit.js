...
const checkoutBranch = function(repo, branchName, strategy) {
    let options = new git.CheckoutOptions();
    options.checkoutStrategy = strategy;

    return repo.getReference(branchName)
    .then((branchRef) => repo.checkoutBranch(branchRef, options));    
}
 ...
 * @param {Promise<void>}
 */
const cleanCheckoutBranch = function(repo, branchName) {
 ...
 */
const forceCheckoutBranch = function(repo, branchName) {
    return checkoutBranch(
        repo,
        branchName,
...
overrideSubModuleMemeberAndWarn(commands, commandsName, forceCheckoutBranch, "forceCheckoutBranch");
overrideSubModuleMemeberAndWarn(commands, commandsName, cleanCheckoutBranch, "cleanCheckoutBranch");
