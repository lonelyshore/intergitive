...
function _compareIndexWithSerializedIndex(index, serializedIndexEntries) {
    ...
    for (let i = 0; i < index.entryCount(); i++) {
        ...
        if (!(entry.id in dict) || !dict[entry.id].equals(entry)) {
            return false;
        }
    }
...
exports.RepoVersionControl = class RepoVersionControl {
    ...
    setTarget(sourcePath, storeName) {
        ...
        let asserts = () => {
            ...
            .then(() => {
                assert(fs.existsSync(sourcePath), `sourcePath ${sourcePath} does not exist`);
    ...
    backup(tagName, isTemplate) {
        ...
        let asserts = () => {
            return Promise.resolve()
            .then(() => {
                assert(fs.existsSync(this.sourcePath), `sourcePath ${this.sourcePath} does not exist`);
                assert(fs.existsSync(this.storePath), `storePath ${this.storePath} does not exist`);
                assert(fs.existsSync(path.join(this.storePath, '.git')), `storePath does not has .git folder`);
            });
        };
    ...
    diffWithTemplate(tagName) {
        ...
        .then(() => {
            if (!areEqual) {
                ...
                return this[_compareWorkingTreeDiff](sourceRepo, referenceRepo)
                .then(result => {
                    if (!result) {
                        areEqual = false;
                    }
    ...
    [_compareWorkingTreeDiff](sourceRepo, referenceRepo) {
        ...
        .then(() => {
            return git.Diff.indexToWorkdir(sourceRepo, sourceIndex, git.Diff.OPTION.INCLUDE_UNTRACKED | git.Diff.OPTION.IGNORE_WHITESPACE_EOL)
            .then(result => {
                sourceDiff = result;
            })
            .then(() => git.Diff.indexToWorkdir(referenceRepo, referenceIndex, git.Diff.OPTION.INCLUDE_UNTRACKED | git.Diff.OPTION.IGNORE_WHITESPACE_EOL))
            .then(result => {
                referenceDiff = result;
            });
        })
        .then(() => {
            return _compareDiffs(sourceDiff, referenceDiff);
        })
    }
}