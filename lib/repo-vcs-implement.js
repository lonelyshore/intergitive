...
class CommitNode {
    ...
    static buildFromCommit(commit, cache) {
        if (commit.sha() in cache) {
            ...
            let newNode = new CommitNode([], commit.treeId());
     ...
     */
    equals(other) {
        return this.treeId.equal(other.treeId) && previousesEquas(other.previouses);
    }
 ...
 * 
 * @param {git.Repository} repo 
 * @param {git.Repository} refRepo 
 */
function _compareRepoRevisions(repo, refRepo) {
    let repoRefs, refRepoRefs;
