...
const _compareHead = Symbol("_compareHead");
...
exports.RepoVersionControl = class RepoVersionControl {
    ...
    [_compareWorkingTreeDiff](sourceRepo, referenceRepo) {
        ...
        let correctSize = (indexEntry) => {
            return fs.stat(path.join(this.backupWorkTreePath, indexEntry.path))
            .then(stat => {
                indexEntry.fileSize = stat.size;
            })
        }
        ...
        .then(() => {
            ...
            .then(index => {
                let correctEntries = Promise.resolve();
                index.entries().forEach(entry => {
                    correctEntries = correctEntries.then(() => correctSize(entry));
                })
