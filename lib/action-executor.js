...
class ActionExecutor {
    ...
    executeCheckout(repoSetupName, refName) {
        return this[operateRepo](
            repoSetupName,
            repo => {
                return git.Commands.cleanCheckoutRef(repo, refName);
            }
        )
    }
            ...
            repo => {
                ...
                .then(result => {
                    if (!result.commitCreated) {
                        return new Error('Failed to create commit');
                    }
                });
            }
        );
