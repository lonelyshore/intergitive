 ...
 */
function calculateCourseUnlockStatus(progress, courseName, courseTree) {

    return progress.getProgress(courseName)
    .then(courseProgress => {
            ...
            (mapping, item) => {
                if (item.children) {
                    mapping[item.id] = item.children.map(
                        child => itemToCompleteness[child.id]
                    )
                    .every(result => result === true);
                }
                else {
                    mapping[item.id] = itemToCompleteness[item.id];
                }
...
let store = {
    state: {
        ...
        progress: new progress.Progress(paths.progressPath)
    },
    get levelState() {
