...
const courseConfig = dependencies.courseConfig;
const { ProgressEnum } = require('../common/progress');
const { ProgressManager, ProgressInfo } = require('./progress-manager');
const COURSE_CONFIG_SCHEMA = dependencies.courseSchema;

const stepConfigs = dependencies.stepConfigs;
const actionConfigs = dependencies.actionConfigs;
const LEVEL_CONFIG_SCHEMA = dependencies.levelSchema;

const ProcessState = stepConfigs.ProcessState;

const api = window.api;
//api.levelSchema = { schema: LEVEL_CONFIG_SCHEMA };
...
function executeActionsSequentially(actions) {
    ...
    actions.forEach(action => {
        thread = thread.then(() => {
            return api.invokeExecute(action, yaml, LEVEL_CONFIG_SCHEMA);
        })
    });
        ...
        .then(genLastCheckpointActionorRepo => {
            let resetPromise = resettingRepos.reduce(
                (acc, resettingRepoSetupName) => {
                    ...
                    return action ?
                        acc.then(() => api.invokeExecute(action)) :
                        acc;
                },
                Promise.resolve()
            );
        ...
        .then(() => {
            ...
            repoSetupNames.forEach(repoSetupName => {
                ...
                executeSaveCheckpoints = executeSaveCheckpoints.then(() => {
                    return api.invokeExecute(action);
                });
            });
