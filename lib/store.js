...
const UserOperablePhase = require('./config-step').UserOperablePhase;
...
let store = {
    ...
    verifyInputAnswer(stepKey, input) {
        ...
        .then(() => {
            stepState.state.phase.current = 
                input === processedAnswer
                ? UserOperablePhase.SUCCESS
                : UserOperablePhase.FAILED;
        });
    }
}
