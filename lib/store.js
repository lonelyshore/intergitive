...
const MutableAssetLoader = require('./asset-loader').MutableAssetLoader;
...
const courseAssetLoader = new MutableAssetLoader(paths.courseResourcesPath);
...
let store = {
    ...
    loadTerms() {
        ...
        Object.keys(this.levelState.terms).forEach(key => {
            loadPromises.push(
                loadCourseAsset.loadCommonString(key, loaderPair)
                .then(term => {
    ...
    loadCommonAssetRelativePaths() {
        ...
        Object.keys(this.levelState.commonAssetRelativePaths).forEach(key => {
            loadPromises.push(
                loadCourseAsset.loadCommonAssetPath(key, loaderPair)
                .then(fullAssetPath => {
    ...
    loadText(assetId) {
        return loadCourseAsset.loadCourseText(assetId, this.state.courseState.courseName, loaderPair);
    },
    processAssetIdInText(text) {
        ...
        .then(replacements => {
            return replacements.reduce(
                (replacementMap, replacement) => {
                    replacement.match(
                        (replacedString, replacingContent) => {
                            replacementMap[replacedString] = replacingContent;
                        },
                        (replacedString, fullAssetPath) => {
                            replacementMap[replacedString] = this.getAssetRelativePath(
                                fullAssetPath
                            );
                        }
                    );
        ...
        .then(replacementMap => {
            return text.replace(utility.matchMustacheAssetId, (matched) => {
    ...
    loadCourse(courseName) {
        ...
        return loadCourseAsset.loadCourse(courseName, loaderPair)
        .then(course => {
    ...
    loadLevel(levelName) {
        ...
        return loadCourseAsset.loadLevelFromCourse(levelName, this.courseState.courseName, loaderPair)
        .then(config => {
