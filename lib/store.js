...
let store = {
    ...
    loadLevel(levelName) {
        ...
        .then(config => {
            ...
            .then(() => {
                ...
                if (repoVcsSetups) {
                    this.state.workingPaths = {};

                    this.state.repoSetupNames = Object.keys(repoVcsSetups);

                    this.state.repoSetupNames.forEach(repoVcsSetupName => {
                        let repoVcsSetup = repoVcsSetups[repoVcsSetupName];

                        this.state.workingPaths[repoVcsSetupName] = 
                            path.join(
    ...
    openWorkingPath(stepKey) {
        return new Promise(resolve => {
            let step = this.state.stepStates[stepKey].step;
            ...
            shell.openItem(this.state.workingPaths[step.repoSetupName]);
    ...
    markProcessComplete(stepKey) {
        ...
        this.state.stepStates[stepKey].state.processState = ProcessState.PROCESS_COMPLETE;
    },
