...
let store = {
    ...
    executeActions(actions, stepKey) {
        let state = this.state.stepStates[stepKey].state;

        assert(state.phase.current !== UserOperablePhase.RUNNING);
        state.phase.current = UserOperablePhase.RUNNING;
        ...
        return thread
        .then(() => {
            state.phase.current = UserOperablePhase.SUCCESS;
            return true;
        })
        .catch(err => {
            state.phase.current = UserOperablePhase.FAILED;
            return false;
        })
    },
    unblock(stepKey) {
        ...
        if (blockingStepArrayIndex != this.state.blockingSteps.length - 1) {
            ...
            this.state.currentBlockingStep = Object.length(this.state.stepStates);
        }
    ...
    markProcessComplete(stepKey) {
        ...
        this.state.stepStates[stepKey].state.processState = stepConfigs.ProcessState.PROCESS_COMPLETE;

        if (index === this.state.minProcessingStep) {
            let i = this.state.minProcessingStep;
            for (; i <= this.state.currentBlockingStep; i++) {
                let stepState = this.state.stepStates[`${i}`];
                if (stepState.step.needProcess) {
                    if (stepState.state.processState === stepConfigs.ProcessState.PROCESSING) {
    ...
    updateBlockingStates() {
        Object.keys(this.state.stepStates).forEach(stepKey => {
            ...
            if (!stepState.state.isBlocked) {
                if (stepState.step.needProcess && stepState.state.ProcessState === stepConfigs.ProcessState.PREPARE_PROCESS) {
                    stepState.state.processState = stepConfigs.ProcessState.PROCESSING;
