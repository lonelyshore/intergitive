...
function initializeRepoStore(storePath, refStoreName, assetLoader) {
    ...
    .then(() => {
        return assetLoader.getFullAssetPath(`assets/archive:${refStoreName}`)
    })
    .then(archivePath => {
...
let store = {
    ...
    markProcessComplete(index) {
        this.state.stepStates[`${index}`].state.processState = stepConfigs.ProcessState.PROCESS_COMPLETE;
