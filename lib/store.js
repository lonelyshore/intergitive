...
class CourseProgress {
    ...
    static create(progressPath) {
        return Promise.resolve(new CourseProgress(progress))
        .then(courseProgress => {
...
let store = {
    state: {
        ...
        progress: new Progress(paths.progressPath)
    },
    get levelState() {
    ...
    loadRepoReference(stepKey) {
        if (!this.levelState.isDebug) return Promise.resolve(new Error('store.loadRepoReference is debug only!'));
        ...
        return Promise.resolve()
        .then(() => {
            let loadRepoRefs = Promise.resolve();
            ...
            repoSetupNames.forEach(repoSetupName => {
                ...
                loadRepoRefs = loadRepoRefs.then(() => {
                    return action.executeBy(this.levelState.actionExecutor);  
                });
            })
    ...
    saveProgress() {
        ...
        .then(progress => {
            return progress.setItemComplete(currentLevel);
        });
    },
    completeLevel() {
        return Promise.resolve()
