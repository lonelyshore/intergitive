...
let store = {
    ...
    loadCommonAssetRelativePaths() {
        ...
        Object.keys(this.levelState.commonAssetRelativePaths).forEach(key => {
            loadPromises.push(
                api.load('loadCommonAssetPath', key)
                .then(fullAssetPath => {
    ...
    loadText(assetId) {
        return api.load('loadCourseText', assetId, this.state.courseState.courseName);
    },
        ...
        .then(genLastCheckpointActionorRepo => {
            let resetPromise = resettingRepos.reduce(
                (acc, resettingRepoSetupName) => {
                    ...
                    return action ?
                        acc.then(() => action.executeBy(this.levelState.actionExecutor)) :
                        acc;
                },
                Promise.resolve()
            );
