...
class AssetLoader {
    ...
    getFullAssetPath(assetPath) {
        ...
        return this[retrieveValueFromIndex](this.bundlePath, parsedPath, ONDISK_SOURCE)
        .then(indexedResult => {
            return path.join(this.rootPath, indexedResult.assetBundlePath, parsedPath.containerSubPath, indexedResult.assetContent);
        })
        .catch(err => {
    ...
    loadInfileAsset(assetPath) {
        return Promise.reject(new Error('AssetLoader.loadInfileAsset is deprecated'));
    }
    ...
    loadTextContent(assetPath, encoding) {
        ...
        return loadInfileContent()
        .catch(error => {
            if (!(error instanceof InternalNotFoundError)) {
                throw error;
            }
            else {
                return this.getFullAssetPath(assetPath)
                .then(fullAssetPath => {
     ...
     * @returns {string} 
     */
    [retrieveValueFromIndex](bundlePath, parsedAssetPath, source) {
        ...
        return readIndex()
            .then(indexResult => {
                if (source === INFILE_SOURCE && assetName in indexResult.infile) {
                    return { 
                        assetContent: indexResult.infile[assetName], 
                        assetBundlePath: bundlePath 
                    };
                }
                else if (source === ONDISK_SOURCE && assetName in indexResult.ondisk) {
                    return { 
                        assetContent: indexResult.ondisk[assetName], 
                        assetBundlePath: bundlePath
                    };
                }
                    ...
                    if (this[bundlePathWalked].includes(newBundlePath)) {
                        ...
                        return this[retrieveValueFromIndex](newBundlePath, parsedAssetPath, source);
                    }
                }
                else {
