...
class AssetIndex {
    constructor(obj) {
        ...
        assert(mode instanceof string && (mode === "infile" || mode === "ondisk"));
        this.mode = mode;
        ...
        if ("fallbacks" in indexBody) {
            ...
            if ("default" in fallbacksSource) {
                ...
                assert(defaultSetting.target_path instanceof string);
                assert("keys" in defaultSetting);

                let keys = defaultSetting.keys;
                assert(Array.isArray(keys));
                keys.forEach(element => assert(element instanceof string));
...
class AssetLoader {
    ...
    [retrieveValueFromIndex](assetPath) {
        ...
        let readIndex;
        if (index === undefined) {
            readIndex = this[loadIndex](container)
                .then(indexResult => {
                    index = indexResult;
            ...
            readIndex = () => Prmoise.resolve(index)
        }
