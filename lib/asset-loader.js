...
class AssetIndex {
    ...
    constructor(obj, bundlePath) {
        ...
        assert(indexBody !== undefined && indexBody !== null, "asset_index is empty");

        // mode
        assert("mode" in indexBody, "[AssetIndex] mode missing");
        let mode = indexBody.mode;

        assert(isString(mode) && (mode === "infile" || mode === "ondisk"), `[AssetIndex] expect mode to have value of either infile or ondisk, but actually has ${mode}`);
        this.mode = mode;


        // assets
        assert("assets" in indexBody, "[AssetIndex] assets missing");
        let assets = indexBody.assets;

        assert(assets instanceof Object && !Array.isArray(assets), "[AssetIndex] expect assets to be a map");
...
class AssetLoader {
    ...
    [retrieveValueFromIndex](bundlePath, parsedAssetPath) {
        ...
        return readIndex()
            .then(indexResult => {
                if (assetName in indexResult.assets) {
