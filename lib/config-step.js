...
class Step {
    ...
    getSerializableTransformers() {
        return [
            function(state, transformed) {
                transformed.isBlocked = state.isBlocked;
            }
        ];
    }

    getDeserializeTransformers() {
        return [
            function(deserialized, state) {
                state.isBlocked = deserialized;
            }
        ]
    }
}
...
const interactable = Sup => class extends Sup {
    ...
    getSerializableTransformers() {
        return super.getSerializableTransformers()
        .push(
            function(state, transformed) {
                return transformed.isInteractable = state.isInteractable;
            }
        );
    }

    getDeserializeTransformers() {
        return super.getDeserializeTransformers()
        .push(
            function(deserialized, state) {
                state.isInteractable = deserialized.isInteractable;
            }
        )
    }
}
...
const renderable = (Sup, componentType) => class extends Sup {
    ...
    getDeserializeTransformers() {
        ...
        .push(
            function(deserialized, state) {
                state.isEnabled = deserialized.isEnabled;
            }
        );
    }
}
...
const hasPhase = (Sup, PhaseType) => class extends Sup {
    ...
    getSerializableTransformers() {
        ...
        .push(
            function(state, transformed) {
                transformed.phase = state.phase.current.toString();
            }
        )
    }
    ...
    getDeserializeTransformers() {
        ...
        .push(
            function(deserialized, state) {
                state.phase.current =
                    convertStepPhaseFromString(deserialized.phase, PhaseType.PHASES);
            }
        )
    }
};
...
const needProcess = Sup => class extends blocking(Sup) {
    ...
    getSerializableTransformers() {
        ...
        .push(
            function(state, transformed) {
                transformed.processState = state.processState.current.toString();
            }
        );
    }
        ...
        .push(
            function(deserialized, state) {
                state.processState.current = 
                    convertStepPhaseFromString(
                        deserialized.processState,
                        ProcessState.PHASES
                    );
            }
        )
