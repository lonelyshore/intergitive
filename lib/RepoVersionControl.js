    ...
    backup(tagName) {
        ...
        let commitAndTag = () => {
            ...
            .then((oidResult) => {
                ...
                .then((ref) => {
                    masterRef['refName'] = ref.name();
                    return repo.getReferenceCommit(ref.name());
                })
                .then((commit) => {
                    masterRef['commitOid'] = commit.id();
                    return masterRef;
                })
            })
            .catch((err) => {
            ...
            .then((baseRef) => {
                return repo.createCommit(baseRef.refName, _signature(), _signature(), tagName, oid, baseRef.commitOid ? [baseRef.commitOid] : []);
            })
            .then((commitOid) => {
