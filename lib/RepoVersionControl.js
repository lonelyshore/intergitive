...
exports.RepoVersionControl = class RepoVersionControl {
    ...
    backup(tagName) {
        ...
        let commitAndTag = () => {
            ...
            .then((oidResult) => {
                oid = oidResult;
                return git.Reference.nameToId(repo, "master")
                .then((masterId) => {
                    return { refName: "master", refId: masterId };
                });
            })
            .then((baseRef) => {
                return repo.getCommit(baseRef.refId).then((oid) => {
                    baseRef['oid'] = oid;
                    return baseRef;
                });
            })
            .catch((err) => {
                var dummy = err;
                return { refName: "HEAD", oid: undefined };
            })
            .then((baseRef) => {
                return repo.createCommit(baseRef.refName, _signature(), _signature(), tagName, oid, baseRef.oid ? [baseRef.oid] : []);
            })
            .then((commitOid) => {
