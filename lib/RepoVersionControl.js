                ...
                if (repo.isEmpty() && index.entryCount() != 0) {
                    ...
                    .then((diff) => {
                        if (diff.numDeltas() != 0) {
                            return index.writeTree()
                            .then((indexTreeOid) => {
                                return repo.createCommit("master", _signature(), _signature(), tagName, indexTreeOid, [baseCommit]);
                            });
                        }
                        else {
                            console.log("Not commiting for tag " + tagName);
                            return baseCommit.id();
                        }
                    })
                }
            });
