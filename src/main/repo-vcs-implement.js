...
class CommitNode {
  ...
  static buildFromCommit (commit, cache) {
    if (commit.sha() in cache) {
      ...
      const newNode = new CommitNode([], commit.treeId().tostrS())
      ...
      if (parentCount === 0) {
        ...
        const previouses = newNode.previouses
        const getPreviouses = []
...
function _compareIndexWithSerializedIndex (index, serializedIndexEntries) {
  ...
  for (let i = 0; i < index.entryCount(); i++) {
    const entry = index.getByIndex(i)
    const samePaths = dict[entry.path]

    if (!samePaths) {
      return false
    } else {
      const serializedEntry = samePaths[entry.id]
...
async function sourceTextEquivalentToRefText (sourceTextPath, refTextPath) {
  ...
  const consumeBuff = (buff, lines) => {
    while (buff.length !== 0) {
      const line = buff.shift()
  ...
  const sourceStream = fs.createReadStream(sourceTextPath)
  sourceStreamClosed = false

  const refStream = fs.createReadStream(refTextPath)
...
function _dirtyWorkTreeEqualRefWorkTree (sourceDiff, sourcePath, refPath) {
  ...
  for (let i = 0; i < sourceDiff.numDeltas(); i++) {
    const diff = sourceDiff.getDelta(i)
    const diffFilePath = diff.newFile().path()
...
module.exports.RepoVersionControl = class RepoVersionControl {
  ...
  diffWithTemplateLocal (tagName) {
    ...
    return fs.exists(path.join(this.sourcePath, '.git'))
      ...
      .finally(() => {
        try {
          if (sourceRepo) { sourceRepo.cleanup() }

          if (referenceRepo) { referenceRepo.cleanup() }
        } catch (e) {
          console.error(e)
        }
      })
        ...
        return referenceRepo.refreshIndex()
          .then(refIndex => {
            refIndex.entries().forEach(entry => {
              const correctSize = correctSizeMap['wrapper/' + entry.path]
