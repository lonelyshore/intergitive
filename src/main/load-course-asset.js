...
class LoaderPair {
  ...
  loadCourseRaw (courseName, language) {
    const loader = this.getGlobalLoader(language)
...
function createCourseAssetLoaderPair (courseStruct) {
  const globalLoader = new AssetLoader(courseStruct.resourcesPath)
  const courseLoader = new AssetLoader(courseStruct.courseResourcesPath)
