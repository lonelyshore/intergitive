...
const coursePlayableExecutionStep = (isAutoStart) => {
    return class extends blockingExecutionStep(
        mayAppendCheckpointStep(hasActionsStep(Step)),
        isAutoStart
    ) {
        constructor(descriptionId, actions, appendCheckpoint, resettingRepos) {
            super();
            this.actions = actions;
            this.descriptionId = descriptionId;
            this.appendCheckpoint = appendCheckpoint;
            this.resettingRepos = resettingRepos || [];
        }
        ...
        getDescription(store) {
            return this.descriptionId ? 
                store.loadText(this.descriptionId).then(result => store.processAssetIdInText(result)) :
         ...
         */
        takeAction(store, stepKey) {
            return store.resetReposToLatestCheckpoint(
                this.resettingRepos,
                stepKey
            )
            .then(() => store.executeActions(this.actions));
        }
    }
};

class ExecuteActionStep extends coursePlayableExecutionStep(false) {
    constructor(descriptionId, actions, appendCheckpoint, resettingRepos) {
        super(descriptionId, actions, appendCheckpoint, resettingRepos);
        this.klass = 'ExecuteActionStep';
    }
}

class AutoPlayActionsStep extends coursePlayableExecutionStep(true) {
    constructor(descriptionId, actions, appendCheckpoint, resettingRepos) {
        super(descriptionId, actions, appendCheckpoint, resettingRepos);
        this.klass = 'AutoPlayActionsStep';
    }
}
